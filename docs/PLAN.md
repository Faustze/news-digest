...
| C-04 | Добавить unit-тесты (pytest) | ✅ (116 тестов) |
...
| B-07 | PostgreSQL-персистентность (Alembic, репозитории) | STATE.md | ✅ (2026-08-18) |
| B-08 | Связка Web UI с персистентностью | STATE.md | ☐ |
...
| D-02 | Настроить GitHub Actions CI/CD для деплоя Web UI на GitHub Pages | ☐ (удалён 2026-08-18: `news.faustze.tech` сервит из корня, а билд линковал `/news-digest/` → 404) |
...
> Примечание: изначально «нет базы данных» (профиль/feedback в JSON). Слой PostgreSQL (Alembic + репозитории) добавлен позже как локальный инструмент разработки и не является обязательным для пайплайна.
